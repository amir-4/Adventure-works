...
-- Create forgien key
IF EXISTS (SELECT * FROM SYS.tabels 
             WHERE NAME = 'fact_sales')
ALTER TABLE fact_sales
ADD CONSTRAINT FK_fact_sales_dim_product FOREIGN KEY (product_key) REFERENCES
dim_product(product_key);

-- Dropping and Create Index
IF EXISTS (SELECT * FROM SYS.INDEX 
...
IF EXISTS (SELECT * FROM SYS.INDEX 
           WHERE NAME = 'dim_product_Category' AND OBJECT_ID = OBJECT_ID('dim_product'))
DROP INDEX dim_product.dim_product_product_Category
CREATE INDEX dim_product_Category ON dim_product(product_category) 
